...
def galactic_position(i: int, dist_pc: float, seed: int) -> tuple:
    """
    Places a star using a realistic galactic disk distribution.
    - Stars spread in a flat disk (like the Milky Way)
    - Thin disk: most stars within ~15 degrees of galactic plane
    - Scale radius ~30 scene units, scale height ~4 units
    - Uses star index + distance as seed for deterministic but natural scatter
    """
    def rng(s):
        x = math.sin(s * 127.1 + i * 311.7) * 43758.5453
        return x - math.floor(x)

    # Galactic angle — full 360 degrees, clustered in spiral arm directions
    base_angle = rng(seed) * math.pi * 2
    # Slight spiral arm bias (2 arms offset by pi)
    arm_offset = (i % 2) * math.pi
    arm_pull   = math.sin(base_angle * 2 + arm_offset) * 0.3
    theta      = base_angle + arm_pull

    # Radial distance from origin — exponential disk profile
    # Closer stars (lower dist_pc) stay nearer origin
    norm_dist  = min(dist_pc / 500, 1.0)
    r          = 5 + norm_dist * 70 + rng(seed + 1) * 12

    # Galactic plane position (x, z)
    x = r * math.cos(theta)
    z = r * math.sin(theta)

    # Height above/below plane — thin disk (scale height ~5% of radius)
    # Most stars very close to plane, occasional outliers
    scale_h = max(r * 0.06, 2)
    gauss   = math.sqrt(-2 * math.log(max(rng(seed + 2), 1e-9))) * math.cos(2 * math.pi * rng(seed + 3))
    y       = gauss * scale_h
...
def assign_positions(stars: list[dict]) -> list[dict]:
    """
    Assign 3D positions using a realistic galactic disk distribution.
    Stars spread in a flat plane like the Milky Way, with Solar System
    at origin. Closer stars stay nearer the center.
    """
    for i, s in enumerate(stars):
        x, y, z = galactic_position(i, s['dist_pc'], seed=i * 7)
        s['x'] = x
        s['y'] = y
        s['z'] = z
    return stars
...
//  {len(clean)} stars · Diverse habitability · Galactic disk positions
